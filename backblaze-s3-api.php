...
class BackblazeS3 {
	...
	// Transient prefix
	private $transient_prefix = 'b2-region-';
	...
	private function get_host() {
		...
		switch ( $region ) {
			case 'us-east-001' :
				return 's3.us-east-001.backblazeb2.com';
				break;
			case 'us-east-002':
				return 's3.us-east-002.backblazeb2.com';
				break;
			case 'eu-central-001' :
				return 's3.eu-central-001.backblazeb2.com';
				break;
			case 'eu-central-002' :
				return 's3.eu-central-002.backblazeb2.com';
				break;
			case 'eu-central-003' :
				return 's3.eu-central-003.backblazeb2.com';
				break;
			case 'us-west-001' :
				return 's3.us-west-001.backblazeb2.com';
				break;
			case 'us-west-002' :
				return 's3.us-west-002.backblazeb2.com';
				break;
			case 'us-west-003' :
				return 's3.us-west-003.backblazeb2.com';
				break;
			case 'us-west-004' :
			...
			default:
				// Unrecognized region
				$this->log( sprintf( esc_html__( 'An unrecognized B2 region (%s) was provided.', 'wc_backblaze_b2' ), $region ) );
	...
	private function get_remote_region() {
		...
		$region = 'eu-central-003'; // default

		if ( $this->use_path_style ) {
			$url = 'https://s3.'. $region .'backblazeb2.com/' . $this->bucket . '?';
		} else {
			$url = 'https://' . $this->bucket . '.s3.'. $region .'.backblazeb2.com/?';
		}
	...
	public function log( $message ) {
		...
		$this->log->add( 'backblaze-b2', $message );
	}
}
